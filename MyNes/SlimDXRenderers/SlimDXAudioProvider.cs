...
namespace MyNes
{
    class SlimDXAudioProvider : IAudioProvider
    {
        ...
        private byte buffer_playback_currentSample_l;
        private byte buffer_playback_currentSample_r;
        private int buffer_freq;
        private double target_fps;
        private int block_alighn;
        ...
        public void Initialize()
        {
            ...
            wav.Channels = (short)MyNesMain.RendererSettings.Audio_Channels;
            //wav.BitsPerSample = (short)MyNesMain.RendererSettings.Audio_BitsPerSample;
            wav.BitsPerSample = 8;
            ...
            block_alighn = wav.BlockAlignment;
            stereo_mode = MyNesMain.RendererSettings.Audio_Channels == 2;
            stereo_nes_mode = MyNesMain.RendererSettings.Audio_StereoNes;

            BufferSize = MyNesMain.RendererSettings.Audio_PlaybackBufferSizeInKB * 1024;
            ...
            des.Flags = BufferFlags.ControlVolume | BufferFlags.GlobalFocus;
        ...
        public void ShutDown()
        {
            ...
            buffer_internal = new byte[buffer_internal_size][];
            for (int i = 0; i < buffer_internal_size; i++)
            {
                buffer_internal[i] = new byte[2];
            }
            buffer_internal_r_pos = 0;
            buffer_internal_w_pos = 0;
            samples_added = 0;
        ...
        public void SubmitSamples(ref byte[][] nesBuffer, ref int samplesAdded)
        {
            if (!isInitialized)
                return;
            IsRendering = true;
            ...
            for (int i = 0; i < samplesAdded; i++)
            {
                ...
                buffer_internal[buffer_internal_w_pos][0] = nesBuffer[i][0];
                buffer_internal[buffer_internal_w_pos][1] = nesBuffer[i][1];
            ...
            for (buffer_playback_i = 0; buffer_playback_i < buffer_playback_required_samples; buffer_playback_i += block_alighn)
            {
                // Get the sample from the internal buffer
                if (buffer_internal_r_pos >= buffer_internal_size || buffer_internal_r_pos < 0)
                    buffer_internal_r_pos = 0;

                buffer_playback_currentSample_r = buffer_internal[buffer_internal_r_pos][0];
                buffer_playback_currentSample_l = buffer_internal[buffer_internal_r_pos][1];
                ...
                buffer_playback[buffer_playback_w_pos] = buffer_playback_currentSample_r;
                ...
                if (stereo_mode)
                {
                    ...
                    {
                        buffer_playback[buffer_playback_w_pos] = buffer_playback_currentSample_l;
                        buffer_playback_w_pos++;
                        if (buffer_playback_w_pos >= buffer_playback_size)
                            buffer_playback_w_pos = 0;
                    }
                ...
                case EmuRegion.NTSC:
                    {
                        ...
                        fps_nes_missle = 1.0 / ((target_fps / 4) + (target_fps * 2) + 2);
