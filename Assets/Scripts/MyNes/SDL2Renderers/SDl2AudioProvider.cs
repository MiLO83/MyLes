...
namespace MyNes
{
    unsafe class SDl2AudioProvider : IAudioProvider
    {
        ...
        private long w_pos;
        private long r_pos;
        private byte sample_l;
        private byte sample_r;
        ...
        public void Initialize()
        {
            ...
            SDL2Settings sdl_settings = new SDL2Settings();
            ...
            specs.channels = (byte)MyNesMain.RendererSettings.Audio_Channels;
            ...
            block_align = MyNesMain.RendererSettings.Audio_Channels;// channels * (bits_per_sample / 8), we have 8 bits here so just channels

            samples_count = (MyNesMain.RendererSettings.Audio_PlaybackBufferSizeInKB * 4 * 1024) + (buffer_min*2);
        ...
        private void FixSpeed()
        {
            ...
            fps_nes_missle = 1.0 / (target_fps + 11);
        ...
        public void SubmitSamples(ref byte[][] samples, ref int samples_a)
        {
            if (!enabled)
                return;
            if (!ready)
            ...
            {
                sample_r = (byte)(samples[i][0] * volume);
                sample_l = (byte)(samples[i][1] * volume);

                if (!is_rendering)
                {
                    audio_samples[w_pos++ % samples_count] = sample_r;
                    ...
                    if (stereo_mode)
                    {
                        ...
                        {
                            audio_samples[w_pos++ % samples_count] = sample_l;
                        }
                    }
                    samples_added++;
                }
