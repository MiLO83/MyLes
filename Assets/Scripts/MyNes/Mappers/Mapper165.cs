...
namespace MyNes.Core
{
    ...
    class Mapper165 : Board
    {
        ...
        internal override void ReadCHR(ref ushort address, out byte data)
        {
            ...
            {
                latch_a = 0xFD;
                base.Switch04KCHR(chr_reg[0] >> 2, CHRArea.Area0000);
            }
            else if ((address & 0x1FF0) == 0x0FE0 && latch_a != 0xFE)
            {
                latch_a = 0xFE;
                base.Switch04KCHR(chr_reg[1] >> 2, CHRArea.Area0000);
            }
            else if ((address & 0x1FF0) == 0x1FD0 && latch_b != 0xFD)
            {
                latch_b = 0xFD;
                base.Switch04KCHR(chr_reg[2] >> 2, CHRArea.Area1000);
            }
            else if ((address & 0x1FF0) == 0x1FE0 && latch_b != 0xFE)
            {
                latch_b = 0xFE;
                base.Switch04KCHR(chr_reg[4] >> 2, CHRArea.Area1000);
            }
