...
namespace MyNes.Core
{
    ...
    class Mapper118 : Board
    {
        ...
        internal override void ReadNMT(ref ushort address, out byte value)
        {
            switch ((address >> 10) & 3)
            {
                case 0: value = NMT_RAM[(chr_reg[flag_c ? 2 : 0] >> 7) & 1][address & 0x3FF]; break;
                case 1: value = NMT_RAM[(chr_reg[flag_c ? 3 : 0] >> 7) & 1][address & 0x3FF]; break;
                case 2: value = NMT_RAM[(chr_reg[flag_c ? 4 : 1] >> 7) & 1][address & 0x3FF]; break;
                case 3: value = NMT_RAM[(chr_reg[flag_c ? 5 : 1] >> 7) & 1][address & 0x3FF]; break;
        ...
        internal override void WriteNMT(ref ushort address, ref byte data)
        {
            switch ((address >> 10) & 3)
            {
                case 0: NMT_RAM[(chr_reg[flag_c ? 2 : 0] >> 7) & 1][address & 0x3FF] = data; break;
                case 1: NMT_RAM[(chr_reg[flag_c ? 3 : 0] >> 7) & 1][address & 0x3FF] = data; break;
                case 2: NMT_RAM[(chr_reg[flag_c ? 4 : 1] >> 7) & 1][address & 0x3FF] = data; break;
                case 3: NMT_RAM[(chr_reg[flag_c ? 5 : 1] >> 7) & 1][address & 0x3FF] = data; break;
            }
        }
