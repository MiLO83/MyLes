...
namespace MyNes.Core
{
    ...
    {
        public static void Initialize(bool setupRenderers, string app_path, string app_working_folder)
        {
            Tracer.WriteLine("Initializing My Nes Core ....");
            AppPath = app_path;
            WorkingFolder = app_working_folder;

            Tracer.WriteLine("Loading emu settings ...");

            EmuSettings = new EmuSettings();
            ...
            string[] files = Directory.GetFiles(AppPath, "*", SearchOption.AllDirectories);
        ...
        public static void RecordWave()
        {
            if (NesEmu.ON && NesEmu.SoundEnabled)
            {
                ...
                WaveRecorder.Record(file, (short)RendererSettings.Audio_Channels, 8, RendererSettings.Audio_Frequency);
            }
            else
            {
                if (VideoProvider != null)
